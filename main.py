...
def handle_message (event):
    ...
    if(target.willImg == 0):
        ...
        if target.quickply == 0:
            LineBot_api.reply_message(event.reply_token, TextSendMessage(text=target.msg_array)) 
        elif target.quickply == 1:
            LineBot_api.reply_message(event.reply_token, TextSendMessage(text=target.msg_array,quick_reply = target.reply)) 
            target.quickply = 0
            target.reply = ''
        target.msg = ""
        target.msg_array = []
